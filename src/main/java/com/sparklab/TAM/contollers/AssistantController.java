...
import com.sparklab.TAM.services.EncryptDecryptAssistantKeyService;
...
public class AssistantController {
    ...
    private final EncryptDecryptAssistantKeyService encryptDecryptAssistantKeyService;
    ...
    public ThreadDtoResponse createThread(@RequestBody String message,
        ...
        return openAIService.createThread(message, type, apartmentId, String.valueOf(userId));
    ...
    @GetMapping("TAM/hostFile/{userId}")
    public void createHostFile(@PathVariable String userId) {
        reservationFileService.writeApartmentsToFile(userId);
    }


    @PostMapping("save/AssistantKey")
    public ResponseEntity<?> saveAssistantKey(@RequestParam String assistantKey) {
        return new ResponseEntity<>(encryptDecryptAssistantKeyService.saveAssistantKey(assistantKey), HttpStatus.OK);
    }
