...
import com.sparklab.TAM.dto.ReservationInvoiceDTO;
...
import com.sparklab.TAM.model.*;
import com.sparklab.TAM.repositories.*;
...
//        USE THIS METHOD TO CALL OTHER METHODS EVERY TIME NEW RESERVATIONS ARE MADE
    ...
    private void runAssistant(List<Integer> reservationIds) {
        ...
        for (Integer reservationId : reservationIds) {
            ...
            ThreadDtoResponse threadResponse = openAIService.createThread("Hello, I am communication with you via email", "client", null,null);
...
//                    }
    ...
    public String generateAndSavePdfDocument(ReservationDTO reservationDTO,User user,String paymentTerms)  {
        try {

            List<ApartmentOption>apartmentOptionList=apartmentOptionRepository.getAllApartmentOptionsByTheirIds(apartmentLinkedApartmentOptionRepository.getOptionIdsByApartmentId((long) reservationDTO.getApartment().getId()));
            ReservationInvoice reservationInvoice = new ReservationInvoice();
            reservationInvoice.setInvoiceName(reservationDTO.getEmail()+"-invoice");
            reservationInvoice.setInvoiceType("application/pdf");
            reservationInvoice.setCreatedDate(LocalDateTime.now());
            reservationInvoice.setReservationId((long) reservationDTO.getId());
            byte[] invoice = invoiceGeneratorService.generateInvoice(reservationDTO,user,apartmentOptionList,paymentTerms);
            reservationInvoice.setInvoice(invoice);
            reservationInvoiceRepository.save(reservationInvoice);
