...
public class ClientAssistantService {
    ...
    public AssistantCreationRequest createClientAssistant(String fileId) {
        ...
        String instructionsWord = "You are an assistant that provides information about different properties based on the uploaded Word document. Never say the id of apartment on the chat .The price per night is " +
    ...
    public RunAssistantRequest runClientAssistantRequest() {
        ...
        runAssistantRequest.setInstructions("Address user as Redis. Today's date CET is: " + LocalDate.now() + ". Read all the document for every response and every question" +
                ...
                "bullet-points presented the right way. You have two documents the docx document has the info about features of the home, the clientData.txt document has " +
